...
public class ProblemExplorationTest {
	...
	public void setUp() {
		...
		/*
		 * this.wStart.addFact_asASP(new ExtDLPHead(new DLPAtom("order", an_order)));
		 * this.wStart.addFact_asASP(new ExtDLPHead(new DLPAtom()));
		 * this.wStart.addFact_asASP(new ExtDLPHead(new DLPAtom("available",
		 * an_order))); this.wStart.addFact_asASP(new ExtDLPHead(new DLPAtom("user",
		 * a_user))); this.wStart.addFact_asASP(new ExtDLPHead(new DLPAtom("user_data",
		 * the_user_data)));
		 */
	...
	public void allExpandTest() {
		...
		int flag = 0;
		for (ExpansionNode temp : exploration.getExpandedList()) {
			if (temp.getDestination().contains(this.euncompleteRegForm))
				flag = 1;
		...
		for (int j = 0; j < 25; j++)
			this.exploration.expandNode();
		assertEquals(19, this.exploration.getExpandedList().size());

		int flag2 = 0;
		for (ExpansionNode temp : exploration.getExpandedList())
			if (temp.getDestination().contains(this.enotifyFailureNoCloud))
				flag2 = 1;
	...
	public void testLogic() {
		...
		for (ExpansionNode temp : exploration.getExpandedList()) {
			if (temp.getDestination().contains(new ENode(this.notifyFailureNoCloud)))
				flag = 1;
			if (temp.getDestination().contains(new ENode(this.availableInvoiceNoCloud)))
				flag1 = 1;
			if (temp.getDestination().contains(new ENode(this.acceptedOrderNoCloud)))
				flag2 = 1;
			if (temp.getDestination().contains(new ENode(this.regNoCloud)))
				flag3 = 1;
	...
	@Test
	public void testUploadPrivateCloud() {
		this.exploration = new ProblemExploration(this.model, new ArrayList<AbstractCapability>(), this.domain);
		this.exploration.addCapability(NSM);
		this.exploration.addCapability(UOPCS);
		this.exploration.addCapability(GI);
		...
		ArrayList<Token> startTokens1 = new ArrayList<>();

		this.exploration.addToVisit(this.nodeuploadedOnPrivateCloud, startTokens1, 5);
		...
		this.exploration.expandNode();
		this.exploration.expandNode();
		System.out.println(this.exploration.getExpandedList().get(0).getDestination().get(0).getWorldState());
		System.out.println(this.exploration.getExpandedList().get(1).getDestination().get(0).getWorldState());

		assertEquals(2, this.exploration.getExpandedList().size());
	}
}
