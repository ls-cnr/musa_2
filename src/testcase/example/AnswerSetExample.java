...
public class AnswerSetExample {
	...
	public static void test1() {
		...
		/*
		 *
		 * penguin(tweety). parrot(polly). sparrow(sid). broken_wing(sid).
		 * ostrich(olga).
		 */
		...
		/*
		 * bird(X) :- penguin(X). bird(X) :- parrot(X). bird(X) :- sparrow(X). bird(X)
		 * :- ostrich(X).
		 */
		...
		/*
		 * cannot_fly(X) :- penguin(X). cannot_fly(X) :- ostrich(X). cannot_fly(X) :-
		 * broken_wing(X).
		 */
		...
		LinkedList<DLPLiteral> r8_body = new LinkedList<DLPLiteral>();
		r8_body.add(new DLPAtom("bird", new Variable("X")));
		r8_body.add(new DLPNeg(new DLPAtom("cannot_fly", new Variable("X"))));
