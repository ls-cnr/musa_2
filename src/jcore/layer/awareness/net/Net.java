...
public class Net {
	...
	/**
	 * A structure used to associate all the transition with a trigger condition or
	 * final state
	 */
	private HashMap<Transition, Condition> labels;

	/**
	 * A map used to store the remaining hop in a transition ( used to follow the
	 * path to the final place once )
	 */
	...
	/**
	 * Instantiates a new net. This constructor create a new Petrinet through the
	 * class PetriNetConstruction, then it stores all the attributes coming from the
	 * construction.
	 *
	 * @param model
	...
	/**
	 * The starting method to calculate the hop value. Going through the list of all
	 * tokens, it collects all the Multiple token and their dependents, calculates
	 * the hop for normal tokens and then for the construct under all the Multiple
	 * tokens.
	 *
	 * @param tokens
	...
	public int hop(ArrayList<Token> tokens) {
		...
		for (Token token : tokens)
			...
			else if (token instanceof MultipleToken)
	...
	/**
	 * This method calculates the hop for all the token depending from the Multiple
	 * token mToken.
	 *
	 * @param mToken
	 *            the MultipleToken that stores all the dependents to calculate the
	 *            hop
	...
	/**
	 * This is the main method to select a value from the array that stores values
	 * for all branches under the Multiple token.
	 *
	...
	/**
	 * Them main method to calculate the Hop for a token. It starts from a place and
	 * calculates all the Transition until it reaches the final place. If a
	 * Transition encountered through the path has already been visited, the method
	 * stops and adds the remaining path if it's necessary ( for calculating an hop
	 * from a Conditional case).
	 *
	 * @param place
	...
	/**
	 * Checks if an initialOrPlace is already Multiple token. If so it adds a token
	 * to let the following transition to fire.
	 *
	 * @param place
	 *            the place to check
	...
	/**
	 * Method used to remove all the tokens in the parallel OR case when a token
	 * gets to the finalOrPlace.
	 *
	 * @param finalOrPlace
	 *            the final or place
	 * @param tokens
	...
	/**
	 * Removes, from a Petrinet, all the tokens backward from a place to an initial
	 * place.
	 *
	 * @param initial
	 *            the initial place
