...
public class ConfigSelector extends Artifact {
	...
	void getSolution() {
		...
		/*
		 ...
		 for (String x : this.sc.getSuccessNodes()) System.out.println(x);
		 System.out.println("SEQUENCES");
		 for (ArrayList<String> s : this.sc.getSeqs()) System.out.println(s);
	...
	void notifyENode(String expNodeString) {
		...
		try {
			...
			if (expNode.getDestination().size() > 1) {
				/*
				 * Nodo XOR. "Estraggo" il nodo XOR dal nodo vero e proprio e lo chiamo "X" +
				 * hashCode() dei fatti del mondo dello stato del mondo di provenienza (devono
				 * avere nomi univoci).
				 *
				 * Aggiungo ogni destinazione. X -> 1, X -> 2, ecc. . Nel caso in cui qualcuna
				 * di queste destinazioni sia un nodo di successo, viene processato
				 * successivamente.
				 */
				...
				/*
				 * ExpansionNode semplice. Aggiungo dest e se e' soluzione ne tengo conto.
				 */
